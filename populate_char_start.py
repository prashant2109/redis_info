import os, sys, json, shelve, ast, copy
import config
...
class INC_Company_Mgmt(object):
    ...
    def update_table_boundry(self, grid_dct, pno, phcsv_i):
        ...
        for r_c in r_cs:
            ...
            if ddict[r_c]['ldr'] in ['value', 'hch', 'gh', 'vch']:
                        ...
                        chref_xml = '$$'.join(['{0}@{1}'.format(xm, chref) for xm in xml_i])
    ...
    def update_gridids_rawdb(self, company_id, doc_id):
        ...
        conn, cur   = conn_obj.sqlite_connection(db_path)
        read_qry = """ SELECT gridids FROM rawdb; """
        cur.execute(read_qry)
        t_data = cur.fetchall()
        update_rows = [(rows[0].split('#')[:3], rows[0]) for rows in t_data]
        if update_rows:
            update_stmt = """ UPDATE rawdb SET gridids=? WHERE gridids=?; """ 
    ...
    def read_inc_grids_doc_wise(self, cur, conn, db_name, doc_id_lst, company_id, cid_path, inc_project_id):
        ...
        for row_data in res:
            ...
            #print 'GGGGGGGGGGGGGGGGG', gdata_dct 
            #sys.exit()
...
if __name__ == '__main__':
    ...
    ijson = {"company_id":"1604", "db_name":"DataBuilder_1604", "doc_ids":[1]}     #OFG
